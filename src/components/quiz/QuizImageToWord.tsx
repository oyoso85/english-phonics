...
export default function QuizImageToWord() {
  ...
  const handleChoice = async (choice: { label: string }) => {
    ...
    if (choice.label === currentQ.correctAnswer.label) {
      ...
      setScore((s) => s + 1);
      playCorrectSound();
      try { await playAudio(currentQ.correctAnswer.audio); } catch { /* ignore */ }
      ...
      playWrongSound();
      // Play the wrong word's pronunciation if available
      const wrongItem = questions.find((q) => q.correctAnswer.label === choice.label);
      if (wrongItem) {
        try { await playAudio(wrongItem.correctAnswer.audio); } catch { /* ignore */ }
      }
    }
  };
