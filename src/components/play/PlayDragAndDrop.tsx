...
import { loadVocabularyByCategory } from '../../utils/data';
...
// Pick two different vocabulary categories for classification
...
function getBucketLabel(cat: VocabularyCategory): { label: string; emoji: string } {
  const map: Record<string, { label: string; emoji: string }> = {
    'food-ingredients': { label: '음식', emoji: '🍽️' },
    cooking: { label: '요리', emoji: '🍳' },
    animals: { label: '동물', emoji: '🐾' },
    vehicles: { label: '탈것', emoji: '🚗' },
    'body-parts': { label: '몸', emoji: '🖐️' },
  };
  return map[cat] ?? { label: cat, emoji: '📦' };
}
...
export default function PlayDragAndDrop() {
  ...
  const [bucket1, setBucket1] = useState<VocabularyCategory>('food-ingredients');
  ...
  const b1Info = getBucketLabel(bucket1);
  const b2Info = getBucketLabel(bucket2);
